...
VALIDATE $? "Copying user Service File" # copying user service file


systemctl daemon-reload
systemctl enable user &>> $LOGS_FILE
VALIDATE $? "Enabling User Service" # enabling user service

# cp $SCRIPT_DIR/user.service /etc/systemd/system/user.service
# VALIDATE $? "Copy user service" # copying user service file
...
## User service and route 53 record for user is also created by using the aws cli in the shell-scripting 